...
const accounts = require('../Accounts/Acc');
...
class Bot extends EventEmitter {
    constructor(botid) {
        ...
        this.name = "b" + botid;
        this.botid = botid;
        this.home = path.join(__dirname, "..", "..", "Instances", this.name)

        if (!USER.SUPPORTS_FJ_NET)
            child_process.execSync("./Bash/Create " + this.botid)
    ...
    spawnSteam() {
        ...
        self.logSteam = fs.createWriteStream('./Logs/Steam/' + self.name + '.log');
    ...
    spawnGame() {
        ...
        self.logGame = fs.createWriteStream('./Logs/Game/' + self.name + '.log');
    ...
    full_stop() {
        ...
        if (!USER.SUPPORTS_FJ_NET && fs.existsSync(`/var/run/netns/catbotns${this.botid}`))
            child_process.execSync(`./Bash/Delete ${this.botid}`)
